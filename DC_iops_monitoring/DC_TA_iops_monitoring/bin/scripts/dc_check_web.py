...
def my_url_check():
        ...
        url_cfg_file = ("/opt/splunk/etc/apps/DC_TA_iops_monitoring/bin/scripts/url.conf")
        up_status_file = datetime.datetime.now().strftime("/opt/splunk/etc/apps/DC_TA_iops_monitoring/bin/scripts/logs/"'%H_%M_%d_%m_%Y.%f_up_status.log') 
        down_status_file = datetime.datetime.now().strftime("/opt/splunk/etc/apps/DC_TA_iops_monitoring/bin/scripts/logs/"'%H_%M_%d_%m_%Y.%f_down_status.log')
        time.sleep(2)
        #Create new files 
