...
#FILENAME:dc_ping_server_0.4.py
...
with open('/opt/splunk/etc/apps/DC_TA_iops_monitoring/data/output_status.csv', 'w', newline='') as outcsv:
...
with open('/opt/splunk/etc/apps/DC_TA_iops_monitoring/lookups/servers.csv', 'r', newline='') as csvinput:
#with open('myfile', 'r', newline='') as csvinput:
    ...
    for rows in reader:
        ...
        if status == 'Down':
            ip = 'Not Found'
        with open('/opt/splunk/etc/apps/DC_TA_iops_monitoring/data/output_status.csv', 'a', newline='') as csvoutput:
